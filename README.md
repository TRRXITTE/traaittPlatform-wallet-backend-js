...
Provides an interface to the traaittPlatform network, allowing wallet applications to be built.
...
`npm install traaittplatform-wallet-backend --save`

Yarn:

`yarn add traaittplatform-wallet-backend`
...
`npm install https://github.com/traaittplatform/traaittplatform-wallet-backend-js --save`

Yarn:

`yarn add https://github.com/traaittplatform/traaittplatform-wallet-backend-js`

## Documentation

[You can view the documentation here](https://traaittplatform.github.io/traaittplatform-wallet-backend-js/classes/_walletbackend_.walletbackend.html)

You can see a list of all the other classes on the right side of the screen.
Note that you will need to prefix them all with `WB.` to access them, if you are not using typescript style imports, assuming you imported with `const WB = require('traaittplatform-wallet-backend')`.

## Quick Start

You can find an [example project in the examples](https://github.com/traaittplatform/traaittplatform-wallet-backend-js/tree/master/examples/example1) folder.
...
```typescript
import { WalletBackend, Daemon, IDaemon } from 'traaittplatform-wallet-backend';

(async () => {
    const daemon: IDaemon = new Daemon('127.0.0.1', 24496);
...
* Allow passing custom `checkRingSignatures` to `traaittplatform-utils`
* Bump `traaittplatform-utils` to support passing `checkRingSignatures`

### v4.0.7

* Bump `traaittplatform-utils` to fix issue where transaction signatures would be misordered when supplying a custom `generateRingSignatures` function

### v4.0.6

* Bump `traaittplatform-utils` to fix issue in some environments
...
* Update `traaittplatform-utils` dependency
...
* API change - You must now provide a config to the Utilities/ValidateParameters functions if you are using a non default config, for example if you are using the library for another cryptocurrency. Otherwise, the default traaittPlatform config will be used.
...
`git clone https://github.com/traaittplatform/traaittplatform-wallet-backend-js.git`

`cd traaittplatform-wallet-backend`
